...
void ComputeProcTimes()
{
	...
	{	
		...
		if(i == -1)
		{
			printf("|--%d--",iCurrentTime);	//if CPU is idle
			...
			procInfo[i].iTBT = 0;	//Extra line from FCFS
			cnt++;
			printf("|%d--%s--%d",procInfo[i].iStartTime,procInfo[i].szProcName,procInfo[i].iFinishTime);
        	}	
	}
	printf("|\n");	
}
